...
void PartitionWeight(idx_t * nvtxs, idx_t * ncon, vidx_t& xadj, vidx_t& adjncy, vidx_t& vwgt, vidx_t& vsize,
    vidx_t& adjwgt, vreal_t &tpwgts, vreal_t& ubvec, idx_t * nparts, vidx_t& part)
{
    ...
    if ( *nparts > 8)
    {
        ...
        METIS_PartGraphKway(nvtxs, ncon, xadj.data(), adjncy.data(), vwgt.data(), vsize.data(), adjwgt.data(),
            nparts, tpwgts.data(), ubvec.data(), options, &objval, part.data());
    }
    else
    {
        std::cout << "Using Recursive Partitioning!\n";
        METIS_PartGraphRecursive(nvtxs, ncon, xadj.data(), adjncy.data(), vwgt.data(), vsize.data(), adjwgt.data(),
            nparts, tpwgts.data(), ubvec.data(), options, &objval, part.data());
    }
...
int main(int argc, char *argv[])
{
    ...
    std::fstream file;
    std::string filename = "../example_d.graph";
    file.open(filename);
    std::string line;
    ...
    idx_t ncon = 0;
    char fmtstr[256];
    int nfields = std::sscanf(line.c_str(), "%I64d %I64d %I64d %I64d", &nvtxs, &nedges, &fmt, &ncon);

    std::sprintf(fmtstr, "%03I64d", fmt % 1000);
    ...
    PartitionWeight(&nvtxs, &ncon, xadj, adjncy, vwgt, vsize, adjwgt, tpwgts, ubvec, &nparts, part);
