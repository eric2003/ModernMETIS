...
void PartitionWeight1(idx_t nCells, vidx_t& xadj, vidx_t& adjncy, vidx_t& vwgt, vidx_t& adjwgt, int nPart, vidx_t& cellzone)
{
    idx_t   ncon = 1;
    idx_t* vsize = 0;
    real_t* tpwgts = 0;
    ...
    {
        std::cout << "Using K-way Partitioning!\n";
        METIS_PartGraphKway(&nCells, &ncon, xadj.data(), adjncy.data(), vwgt.data(), vsize, adjwgt.data(),
            &nZone, tpwgts, ubvec, options, &objval, cellzone.data());
    }
    else
    {
        std::cout << "Using Recursive Partitioning!\n";
        METIS_PartGraphRecursive(&nCells, &ncon, xadj.data(), adjncy.data(), vwgt.data(), vsize, adjwgt.data(),
            &nZone, tpwgts, ubvec, options, &objval, cellzone.data());
    }
    std::cout << "The interface number: " << objval << std::endl;
    std::cout << "Partition is finished!\n";
...
void PartitionWeight2(idx_t nCells, vidx_t& xadj, vidx_t& adjncy, vidx_t& vwgt, vidx_t& vsize, vidx_t& adjwgt, int nPart, vidx_t& cellzone)
{
    idx_t   ncon = 1;
    real_t* tpwgts = 0;
    real_t* ubvec = 0;
...
int main(int argc, char *argv[])
{
    ...
    //PartitionWeight1(nvtxs, xadj, adjncy, vwgt, adjwgt, nparts, part);
    PartitionWeight2(nvtxs, xadj, adjncy, vwgt, vsize, adjwgt, nparts, part);
