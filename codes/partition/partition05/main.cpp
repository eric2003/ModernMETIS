...
void PartitionWeight(idx_t nCells, vidx_t& xadj, vidx_t& adjncy, vidx_t& vwgt, vidx_t& vsize, vidx_t& adjwgt, vreal_t &tpwgts, int nPart, vidx_t& cellzone)
{
    idx_t   ncon = 1;
    real_t* ubvec = 0;
    ...
    if (nZone > 8)
    {
        ...
        METIS_PartGraphKway(&nCells, &ncon, xadj.data(), adjncy.data(), vwgt.data(), vsize.data(), adjwgt.data(),
            &nZone, tpwgts.data(), ubvec, options, &objval, cellzone.data());
    }
    else
    {
...
int main(int argc, char *argv[])
{
    ...
    PartitionWeight(nvtxs, xadj, adjncy, vwgt, vsize, adjwgt, tpwgts, nparts, part);
